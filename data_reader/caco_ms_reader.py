...
class RemoteImagesDataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == None:
            positive = None
            return self.__read_original_image__(sample), class_label
        elif mode == "seco".upper():
            image = self.__read_original_image__(sample)
            argumented_positive = self.__read_image__(sample)
            temporal_positive = self.__read_positive__(sample)
            sample = {"image": image, "argumented_positive": argumented_positive, "temporal_positive": temporal_positive}
            return image, argumented_positive, temporal_positive, class_label

        # Read the Main img and the argumented version of original image
        elif mode == "moco".upper():
            image = self.__read_image__(sample)
            argumented_positive = self.__read_positive__(sample)
            sample = {"image": image, "argumented_positive": argumented_positive}
            return image, argumented_positive, class_label

        # Read the Main img and the main image in another time
        elif mode == "temporal".upper():
            image = self.__read_image__(sample)
            temporal_positive = self.__read_positive__(sample)
            sample = {"image": image, "temporal_positive": temporal_positive}
            return image, temporal_positive, class_label


        # negative = self.__read_negative__(sample)
        
        # sample = {"image": image, "temporal_positive": temporal_positive, "negative": negative, "class_label": class_label}
...
class MSReadData():
    ...
    def create_dataLoader(self, dataroot, image_size, batch_size=16, train=True, mode="temporal", shuffle=True, pin_memory=True, num_workers=8):
        self.datas = RemoteImagesDataset(dataroot, batch_size, image_size, train=train, mode=mode)
        ...
        if train:
            ...
            self.dataloader_train = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers, persistent_workers=True)
            self.dataloader_val = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers, persistent_workers=True)
            return self.dataloader_train, self.dataloader_val
        else:
            self.dataloader = torch.utils.data.DataLoader(self.datas, batch_size=batch_size, shuffle=shuffle, pin_memory=pin_memory, num_workers=num_workers, persistent_workers=True)
            return self.dataloader
    
if __name__ == "__main__":
    path = r".\data\single_caco10k\clean_10k_geography"
    dataset = MSReadData()
    batch_size = 10
    img_size=64
    ...
    if train:
        dataloader, val_dataloader = dataset.create_dataLoader(path, img_size, batch_size, train=train, mode=mode.upper(), num_workers=1)
    else:
